<template>
  <aside
    :class="[
      'flex flex-col bg-[var(--color-sidebar-bg)] text-[var(--color-sidebar-text)] transition-all duration-300',
      collapsed ? 'w-16' : 'w-64',
    ]"
  >
    <!-- Logo -->
    <div class="flex items-center h-16 px-4 border-b border-white/10">
      <div class="flex items-center gap-3 overflow-hidden">
        <div class="w-8 h-8 rounded-lg bg-[var(--color-sidebar-active)] flex items-center justify-center text-white font-bold text-sm shrink-0">
          C
        </div>
        <span v-if="!collapsed" class="font-semibold text-white whitespace-nowrap">
          Claude Code Hub
        </span>
      </div>
    </div>
          ...
          <router-link
            ...
            :class="[
              ...
              isActive(item.route)
                ...
                : 'hover:bg-white/10 text-[var(--color-sidebar-text)]',
            ]"
            :title="collapsed ? $t(item.labelKey) : undefined"
          >
            <span class="w-5 h-5 shrink-0 flex items-center justify-center" v-html="item.icon"></span>
    ...
    <div class="p-2 border-t border-white/10">
      <button
        class="w-full flex items-center justify-center p-2 rounded-md hover:bg-white/10 transition-colors"
        @click="$emit('toggle')"
      >
        <svg
