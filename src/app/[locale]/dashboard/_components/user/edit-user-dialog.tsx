...
const EditUserSchema = UpdateUserSchema.extend({
  ...
  priceProviderGroup: z.string().max(200).nullable().optional(),
  latencyProviderGroup: z.string().max(200).nullable().optional(),
...
function buildDefaultValues(user: UserDisplay): EditUserValues {
  return {
    ...
    priceProviderGroup: normalizeProviderGroup(user.priceProviderGroup),
    latencyProviderGroup: normalizeProviderGroup(user.latencyProviderGroup),
      ...
      startTransition(async () => {
        try {
          const userRes = await editUser(user.id, {
            ...
            priceProviderGroup: normalizeProviderGroup(data.priceProviderGroup),
            latencyProviderGroup: normalizeProviderGroup(data.latencyProviderGroup),
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 pt-6 pb-6 space-y-8">
          <UserEditSection
            user={{
              ...
              priceProviderGroup: normalizeProviderGroup(currentUserDraft.priceProviderGroup),
              latencyProviderGroup: normalizeProviderGroup(currentUserDraft.latencyProviderGroup),
              sortStrategy: currentUserDraft.sortStrategy ?? "none",
